...
#2-Sphere
####################
test_sphere <- sphereUnif(320 ,3 ,r=1)
plot3d(test_sphere)

#TDAstats package (ripser)
test.phom.TDAstat <- calculate_homology(test_sphere,dim=1)
plot_barcode(test.phom.TDAstat)

#TDA package (GUDHI)
test.phom.gud <- ripsDiag(X = test_sphere, maxdimension = 1, maxscale = 1.75, library = "GUDHI", printProgress = FALSE)
...
ggplot(Sphere.benchmarks, aes(points, memory)) + geom_point(aes(x = points, y = memory, shape = Algorithm, color = Algorithm)) +
  labs(title="3-Sphere Sample Point Cloud", y="Memory Allocation", x="Number of Points") + theme_bw()
