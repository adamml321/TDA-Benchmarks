...
for (i in 1:10){
sample <- sphereUnif(10+2*i, 1, r=1)

ripser$points[i] <- 10+2*i
Dion$points[i] <- 10+2*i
GUD$points[i] <- 10+2*i

mark.ripser <- mark(calculate_homology(sample,dim=1), min_iterations = 50)
mark.Dion <- mark(ripsDiag(X = sample, maxdimension = 1, maxscale = 1.75, library = "Dionysus", printProgress = FALSE), min_iterations = 50)
mark.GUD <- mark(ripsDiag(X = sample, maxdimension = 1, maxscale = 1.75, library = "GUDHI", printProgress = FALSE), min_iterations = 50)
...
sample19 <- circleUnif(19,r=1)
sample20 <- circleUnif(20,r=1)
sample21 <- circleUnif(21,r=1)

mark.Dion19 <- mark(ripsDiag(X = sample19, maxdimension = 1, maxscale = 1.75, library = "Dionysus", printProgress = FALSE))
mark.Dion20 <- mark(ripsDiag(X = sample20, maxdimension = 1, maxscale = 1.75, library = "Dionysus", printProgress = FALSE))
mark.Dion21 <- mark(ripsDiag(X = sample21, maxdimension = 1, maxscale = 1.75, library = "Dionysus", printProgress = FALSE))
